...
# Load the data
try:
    data = pd.read_csv('singlehop_eval_gpt_unperturbed_incontext_yes_without_rationale.csv')
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    if idx % 10 == 0:
        data.to_csv('singlehop_eval_gpt_unperturbed_incontext_yes_without_rationale.csv', index=False)
