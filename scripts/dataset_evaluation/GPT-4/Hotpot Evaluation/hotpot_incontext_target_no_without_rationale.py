...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_incontext_target_no_old_without_rationale.csv')
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_incontext_target_no_old_without_rationale.csv', index=False)
...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_incontext_target_no_without_rationale.csv')
except FileNotFoundError:
    data = pd.read_csv('../../Datasets/Hotpot_Questions.csv')
    data['Answer_GPT'] = ''
    data['False-Assumption'] = ''
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    # Save the data periodically
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_incontext_target_no_without_rationale.csv', index=False)
