...
openai.api_key = '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'  # Replace with your actual API key
...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_after_eval_gpt_incontext_target_no.csv')
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_after_eval_gpt_incontext_target_no.csv', index=False)
...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_after_eval_gpt_incontext_target_no_old.csv')
except FileNotFoundError:
    data = pd.read_csv('../../Datasets/Hotpot_Questions.csv')
    data['Answer_GPT'] = ''
    data['False-Assumption'] = ''
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    # Save the data periodically
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_after_eval_gpt_incontext_target_no_old.csv', index=False)
