...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_after_eval_gpt_without_incontext_target_yes_old.csv')
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_after_eval_gpt_without_incontext_target_yes_old.csv', index=False)
...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_after_eval_gpt_without_incontext_target_yes.csv')
except FileNotFoundError:
    data = pd.read_csv('../../Datasets/Hotpot_Questions.csv')
    data['Answer_GPT'] = ''
    data['False-Assumption'] = ''
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    # Save the data periodically
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_after_eval_gpt_without_incontext_target_yes.csv', index=False)
