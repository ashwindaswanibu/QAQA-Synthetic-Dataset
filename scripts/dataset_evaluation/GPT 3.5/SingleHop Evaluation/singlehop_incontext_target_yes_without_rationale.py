...
# Load the data
try:
    data = pd.read_csv('singlehop_eval_gpt_incontext_yes_without_rationale_old.csv')
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    if idx % 10 == 0:
        data.to_csv('singlehop_eval_gpt_incontext_yes_without_rationale_old.csv', index=False)
