...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_after_eval_gpt_incontext_yes_without_rationale.csv')
...
for idx in range(start_idx, len(data['New Question'])):
    ...
    false_assumption_question = false_premise_context + " New Question: " + question + "Answer yes if this question contains a false premise, answer no otherwise." +'\n'
    ...
    # Save the data
    data.to_csv('Hotpot_questions_after_eval_gpt_incontext_yes_without_rationale.csv', index=False)
...
# Load the data
try:
    data = pd.read_csv('Hotpot_questions_gpt_incontext_target_yes_old_without_rationale.csv')
except FileNotFoundError:
    data = pd.read_csv('../../Datasets/Hotpot_Questions.csv')
    data['Answer_GPT'] = ''
    data['False-Assumption'] = ''
...
for idx, row in data.iloc[start_idx:].iterrows():
    ...
    if idx % 10 == 0:
        data.to_csv('Hotpot_questions_gpt_incontext_target_yes_old_without_rationale.csv', index=False)
