...
def get_content_from_author1(chat_response):
    try:
        ...
        content = next((c['content']
                       for c in candidates if c['author'] == '1'), None)
        return content
...
for idx in range(len(data)):
    ...
    false_assumption_question = question + \
        'Does this question contain any false premises? Please answer with only Yes/No. Dont elaborate at all' + '\n' + "Answer: "
    response = palm.chat(messages=answer_context + question,
                         temperature=0.1, context="QA")
    yes_no_response = palm.chat(
        messages=yes_no_context + yes_no_question, temperature=0.9, context="QA")
    false_assumption_response = palm.chat(
        messages=false_premise_context + false_assumption_question, temperature=0.9, context="QA")

    # Extract content for author '1'
    answer_content = get_content_from_author1(response)

    false_assumption_content = get_content_from_author1(
    ...
    if idx % 10 == 0:
        data.to_csv(
            'singlehop_complex_palm_with_incontext_yes_without_rationale.csv', index=False)
